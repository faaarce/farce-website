...
const BottomProjects: FC = () => (
  <>
    ...
    <div className={`${rule} grid grid-cols-[1fr_1.2fr_1fr]`}>
      {/* Cyclouse */}
      <div className={`${colR} pr-[18px] pb-5`}>
        <img
          src={IMAGES.cyclouse}
          alt="Cyclouse"
          className={`${imgB} w-full aspect-video object-cover block mb-2.5`}
        />
        <div className="font-extrabold text-[13px] uppercase text-[#2a2a2a] mb-[3px]">
          CYCLOUSE <span className={badgeSm}>NEW</span>
        </div>
        <p className={`${just} text-[11px] text-[#666] leading-[1.55]`}>
          Cycling e-commerce iOS app with Swift, RxSwift, MVVM, Core Data, and
          Alamofire.
        </p>
      </div>
      ...
      {/* Center */}
      <div
        className={`${colR} flex flex-col items-center justify-center text-center px-[18px] py-2.5`}
      >
        <h3 className="font-extrabold text-[clamp(22px,3vw,34px)] uppercase text-[#2a2a2a] leading-none mb-2">
          ALL WORK!
        </h3>
        <div className="w-[30px] h-px bg-[#b5b0a8] mb-2" />
        ...
        <p className={`${lbl} !text-[8px]`}>
          TIP! CLICK ON THE SIDES TO EXPLORE
        </p>
      </div>

      {/* Timepad */}
      <div className="pl-[18px] pb-5">
        <img
